...
def pipeline_features(img, s_thresh, sx_thresh,sxb_thresh):
    #img = np.copy(img)
    #img = mpimg.imread(img)
    LAB = cv2.cvtColor(img, cv2.COLOR_RGB2LAB)
    #GBR = cv2.cvtColor(img, cv2.COLOR_RGB2GBR)
    R_channel = img[:,:,0]
    B_channel = LAB[:,:,2]
    ...
    # Sobel x
    sobelxb = cv2.Sobel(B_channel, cv2.CV_64F, 1, 0) # Take the derivative in x
    abs_sobelxb = np.absolute(sobelxb) # Absolute x derivative to accentuate lines away from horizontal
    scaled_sobelb = np.uint8(255*abs_sobelxb/np.max(abs_sobelxb))
    
    
    # Threshold xb gradient
    sxbbinary = np.zeros_like(scaled_sobelb)
    sxbbinary[(scaled_sobelb >= sxb_thresh[0]) & (scaled_sobelb <= sxb_thresh[1])] = 1
    ...
    #sxbbinary[:, :width//2] = 0 # use the left side
...
#     # For source points I'm grabbing the outer four detected corners
    #src = np.float32([[450,520],[880,520],[1150,720],[170,720] ])
    #src = np.float32([[580,260],[710,260],[1125,710],[195,710] ])
...
def search_around_poly(binary_warped,margin,left_fit,right_fit):
    ...
    return result,left_fit_cr,right_fit_cr,ploty,left_fitx, right_fitx,left_fit,right_fit
...
def measure_offset(leftx, lefty, rightx, righty, left_fit_cr, right_fit_cr, ym_per_pix=30/720, xm_per_pix=3.7/700):
